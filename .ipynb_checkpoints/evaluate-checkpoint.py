...
if __name__ == "__main__":
    ...
    # channel_indices optional if we want <11 channels. If I want e.g. channels 8 and 11, pass vector [7,10]
    train_dataloader, val_dataloader, test_dataloader = get_dataloader(image_dirs, mask_dir, data_transform, mask_transform, display_sample=False, train_percentage=1.0, channel_indices=args.channels)
    
    # Only run if we want to import model
    ...
    if model_name == "Simple":
        ...
        model = SegmentationModel(in_channels=len(args.channels)).to(device)
    ...
    # Call the evaluation function for the model and the test set
